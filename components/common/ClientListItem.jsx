...
const ClientListItem = ({
  ...
  // En mode LTR : les boutons arrivent de la droite (translateX positif → 0)
  // En mode RTL : les boutons arrivent de la gauche (translateX négatif → 0)
  const renderActions = (progress) => {
    const offset = isAr ? -1 : 1;
    ...
    return (
      <View style={[styles.actionsRow, isAr && styles.actionsRowRTL]}>
        {/* Bouton Message */}
        <Animated.View
          style={[
            styles.actionBtn,
  ...
  return (
    <Swipeable
      ref={swipeableRef}
      // LTR : swipe ← → renderRightActions
      // RTL : swipe → → renderLeftActions
      renderRightActions={!isAr ? renderActions : null}
      renderLeftActions={isAr ? renderActions : null}
      overshootRight={!isAr ? false : true}
      overshootLeft={isAr ? false : true}
      friction={2}
      rightThreshold={!isAr ? 40 : undefined}
      leftThreshold={isAr ? 40 : undefined}
      onSwipeableOpen={handleSwipeOpen}
      onSwipeableClose={handleSwipeClose}
    >
      <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  actionsRow: {
    ...
    marginTop: 3,
    gap: scale(6),
    paddingRight: scale(4), // espace côté droit en LTR
  },
  actionsRowRTL: {
    paddingRight: 0,
    paddingLeft: scale(4), // espace côté gauche en RTL
  },
  actionBtn: {
    width: scale(60),
    borderRadius: Radius.md,
